...
const names = (a: Item[]) =>
  Array.from(
    ...
    .join(', ');
function obtainedFrom(value: string, catalog: Item[]) {
  const raw = JSON.parse(value.trim().replace(/^```(?:json)?/i, '').replace(/```$/, '').trim());
  const collection = raw.group ?? raw;
  const entries: Entry[] = collection.cardEntries;
  if (!Array.isArray(entries)) throw Error('No cardEntries were found. Copy the complete data page.');
  const byId = new Map(catalog.map((item) => [item.id, item]));
  const byName = new Map(catalog.map((item) => [norm(item.name), item]));
  const found = new Set<number>();
  entries.forEach((entry) => {
    if (entry.kind === 'npc') return;
    const item = (typeof entry.id === 'number' ? byId.get(entry.id) : undefined) || byName.get(norm(entry.name || entry.cardName));
    if (item) found.add(item.id);
  });
  return catalog.filter((item) => found.has(item.id));
}
function block(
...
export default function Home() {
  const [catalog, setCatalog] = useState<Item[]>([]),
    ...
    [differenceError, setDifferenceError] = useState(''),
    [differenceCopied, setDifferenceCopied] = useState(false);
  useEffect(() => {
  ...
  const compare = () => {
    try {
      if (!catalog.length) throw Error('The item catalog is still loading.');
      const currentIds = new Set(obtainedFrom(compareA, catalog).map((item) => item.id));
      setDifference(names(obtainedFrom(compareB, catalog).filter((item) => !currentIds.has(item.id))));
      setDifferenceError('');
    } catch (e) {
      setDifference('');
      setDifferenceError(e instanceof Error ? e.message : 'Could not compare these collections.');
    }
  };
  const copyDifference = async () => {
    await navigator.clipboard.writeText(difference);
    setDifferenceCopied(true);
    setTimeout(() => setDifferenceCopied(false), 1500);
  };
  const download = () => {
    if (!result) return;
  ...
  return (
    <main>
      <div className="shell">
        ...
        <details className="card compare-card">
          <summary>Compare two collections</summary>
          <p>Find card items in the second collection that are missing from the first.</p>
          <div className="compare-grid">
            <label>
              <span>1 · Current collection</span>
              <textarea value={compareA} onChange={(e) => setCompareA(e.target.value)} placeholder="Paste current collection JSON…" />
            </label>
            <label>
              <span>2 · Other collection</span>
              <textarea value={compareB} onChange={(e) => setCompareB(e.target.value)} placeholder="Paste other collection JSON…" />
            </label>
          </div>
          <button className="primary" disabled={!compareA.trim() || !compareB.trim()} onClick={compare}>
            Find items missing from collection 1
          </button>
          {differenceError && <p className="error">{differenceError}</p>}
          {difference !== '' && (
            <div className="difference-output">
              <textarea className="output-box" readOnly value={difference} />
              <button className="button copy" onClick={copyDifference}>
                {differenceCopied ? <Check size={15} /> : <Clipboard size={15} />}
                {differenceCopied ? 'Copied' : 'Copy comma-separated list'}
              </button>
            </div>
          )}
        </details>
